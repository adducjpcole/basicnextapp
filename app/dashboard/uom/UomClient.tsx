...
export default function UomClient({ uoms }: { uoms: Uom[] }) {
  ...
  function openEdit(uom: Uom) { setFormError(null); setEditing(uom); }
  function openDelete(id: number) { setDeleteError(null); setDeletingId(id); }

  function handleCreate(formData: FormData) {
    startTransition(async () => {
      const result = await createUom(formData);
  ...
  function handleUpdate(formData: FormData) {
    startTransition(async () => {
      const result = await updateUom(formData);
      if (result?.error) { setFormError(result.error); return; }
      setEditing(null);
    });
  }
    ...
    <>
      ...
      <div className="flex items-end justify-between mb-8 border-b border-neutral-200 pb-6">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-neutral-800">Units of Measure</h1>
        </div>
            ...
            {uoms.length === 0 ? (
              ...
              uoms.map((uom) => (
                <tr key={uom.id} className="border-b border-neutral-50 hover:bg-neutral-50 transition-colors group">
                  <td className="px-6 py-4 font-mono text-xs text-neutral-300">{String(uom.id).padStart(2, '0')}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEdit(uom)} className="text-xs font-semibold uppercase tracking-widest text-neutral-400 hover:text-neutral-700 transition-colors">Edit</button>
                      <button onClick={() => openDelete(uom.id)} className="text-xs font-semibold uppercase tracking-widest text-neutral-300 hover:text-blue-500 transition-colors">Delete</button>
                    </div>
                  </td>
                </tr>
              ))
